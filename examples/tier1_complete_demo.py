...
from portfolio.features.factor_models import fetch_fama_french, compute_factor_loadings
...
def main():
    ...
    print("\n   Performance by Regime:")
    print(perf[["regime", "mean_return", "volatility", "sharpe", "frequency"]].to_string(index=False))
    ...
    for ticker in tickers:
        ...
        df_features_dict[f"mom_5d_{ticker}"] = (
            df_returns[ret_col].shift(1).rolling_sum(5)
        )
        # Feature: 20-day volatility
        df_features_dict[f"vol_20d_{ticker}"] = (
            df_returns[ret_col].shift(1).rolling_std(20)
        )
    ...
    for ticker, weight in zip(tickers, weights_cvar):
    ...
    for ticker, weight in zip(tickers, weights_final):
        print(f"   {ticker}: {weight:.2%}")
    ...
    print(f"✅ Fama-French: Computed factor exposures (R² = {loadings['r_squared']:.1%})" if loadings else "⚠️  Fama-French: Skipped (no internet)")
    print("✅ XGBoost: Trained gradient boosting predictor")
    print(f"✅ CVaR: Optimized portfolio ({(cvar_eq - cvar_cvar) / cvar_eq * 100:.1f}% CVaR improvement)")
