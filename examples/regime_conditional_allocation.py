...
def regime_conditional_backtest(
    ...
    sharpe = (
        df_results["portfolio_return"].mean() / df_results["portfolio_return"].std() * np.sqrt(252)
    )
    ...
    perf_by_regime = (
        ...
        .with_columns((pl.col("avg_return") / pl.col("volatility") * np.sqrt(252)).alias("sharpe"))
    )
...
def main():
    ...
    regime_dist = df_regimes.group_by("regime_label").agg(
        pl.col("regime_label").count().alias("count")
    )
    print(regime_dist)
    ...
    static_sharpe = np.mean(static_returns) / np.std(static_returns) * np.sqrt(252)
