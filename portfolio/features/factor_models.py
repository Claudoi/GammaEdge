...
def fetch_fama_french(
    ...
    try:
        ...
        if model == "Carhart4":
            mom_df = pdr.DataReader("F-F_Momentum_Factor_daily", "famafrench", start=start, end=end)[0]
            mom_df = mom_df / 100.0
            df = df.join(mom_df)
...
def compute_factor_loadings(
    ...
    if model == "FF3":
        ...
        factor_cols = ["Mkt-RF", "SMB", "HML", "Mom   "]  # Note: Ken French uses "Mom   " with spaces
        # Standardize to "MOM"
