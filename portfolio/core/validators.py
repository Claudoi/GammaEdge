...
def validate_correlation_matrix(Corr: np.ndarray, tol: float = 1e-6) -> None:
    ...
    if Corr.ndim != 2 or Corr.shape[0] != Corr.shape[1]:
        raise MetricValidationError(f"Correlation matrix must be square, got shape {Corr.shape}")

    # Diagonal should be 1
    diag = np.diag(Corr)
    if not np.allclose(diag, 1.0, atol=tol):
        raise MetricValidationError(f"Correlation matrix diagonal should be 1, got {diag}")
...
def validate_covariance_matrix(Sigma: np.ndarray, tol: float = 1e-10) -> None:
    ...
    if Sigma.ndim != 2 or Sigma.shape[0] != Sigma.shape[1]:
        raise MetricValidationError(f"Covariance matrix must be square, got shape {Sigma.shape}")
