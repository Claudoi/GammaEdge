...
try:
    from app.design_system import (
        ...
        get_global_styles, data_hero_card, metric_grid, section_header
    )
...
try:
    from portfolio.viz.plot_utils import apply_fig_defaults, show_plot
    import plotly.graph_objects as go
...
for page_name, module_name in pages_to_test:
    try:
        ...
        if 'Home' in page_name:
            filepath = f'app/{page_name}'
        else:
            filepath = f'app/pages/{page_name}'
